...
class SocketServer(private val accessibilityService: JeevesAccessibilityService) {
    companion object {
        private const val TAG = "JeevesSocketServer"
    ...
    private fun handlePostRequest(path: String, reader: BufferedReader): String {
        return when {
            ...
            path.startsWith("/tap_by_index") -> {
                handleTapByIndex(reader)
            }
    ...
    private fun handleTapByIndex(reader: BufferedReader): String {
        return try {
            val postData = StringBuilder()
            if (reader.ready()) {
                val char = CharArray(1024)
            ...
            val index = values.getAsInteger("index")
                ?: return createErrorResponse("No index provided")
            ...
            Log.e(TAG, "Error handling tap by index", e)
            createErrorResponse("Failed to tap: ${e.message}")
        }
    }
    ...
    private fun getAccessibilityTree(): String {
        return try {
            ...
            // Use indexed elements for consistency with overlay
            val indexedElements = accessibilityService.getIndexedElements()
            Log.d(TAG, "Found ${indexedElements.size} indexed elements")

            val treeJson = org.json.JSONArray()
            indexedElements.forEach { (index, element) ->
                val elementJson = JSONObject().apply {
                    put("index", index) // This index matches the overlay
                    ...
                    put("enabled", element.nodeInfo.isEnabled)
                    put("content_desc", element.nodeInfo.contentDescription?.toString() ?: "")
                }
                treeJson.put(elementJson)
            }
    ...
    private fun getCombinedState(): String {
        return try {
            // Use indexed elements for consistency
            val indexedElements = accessibilityService.getIndexedElements()
            val treeJson = org.json.JSONArray()
    ...
    private fun buildElementNodeJson(element: ElementNode): JSONObject {
        return JSONObject().apply {
            ...
            element.children.forEach { child: ElementNode ->
