...
class OverlayManager(private val context: Context) {
    ...
    // REMOVED: elementIndexCounter - we now require explicit indices from caller
    ...
    fun setPositionOffsetY(offsetY: Int) {
        Log.d(TAG, "Setting position offset Y to: $offsetY")
    ...
    fun clearElements() {
        elementRects.clear()
        // Force immediate redraw to clear any ghost elements
        handler.post {
    ...
    fun addElement(rect: Rect, type: String, text: String, depth: Int = 0, color: Int = Color.GREEN, index: Int? = null) {
        ...
        // Index is now REQUIRED - no fallback
        val elementIndex = index ?: throw IllegalArgumentException("Element index is required")
    ...
    private fun correctRectPosition(rect: Rect): Rect {
        ...
        if (elementRects.size < 3) {
            Log.d(TAG, "Original rect: $rect, Offset: $positionOffsetY, Corrected: $correctedRect")
        }
